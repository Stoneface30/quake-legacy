...
**Scope:** Collapse the two-tier scatter (`tools/quake-source/` raw clones vs `game-dissection/engines/` dissection outputs) into **one** unified engine folder. Absorb wolfcam's knowledge, then retire the wolfcam-specific source trees per the user directive:
...
4. The wolfcam-specific source trees (`wolfcamql-src`, `wolfcamql-local-src`) are present **both** in `tools/quake-source/` AND as per-tree-variant dirs under `game-dissection/engines/{wolfcamql-src,wolfcamql-local-src}/`. Per `SYNTHESIS.md:38-41`, `wolfcamql-src` is the "GOLDEN TREE for protocol-73 knowledge". We cannot hard-delete it — we must first absorb the delta as a patch series under the unified folder, then delete the whole-tree copy.
5. `game-dissection/wolfcam-knowledge-ingest/` exists only on a sibling worktree (`.claude/worktrees/hopeful-shtern-7fe850/`) with 7 canonical knowledge docs (00-overview through 06-protocol-73-port-plan). This is the "suck all the knowledge" artifact — it needs to land in the unified folder on `main` before wolfcam retires.
...
|---|---|---:|---|
| `tools/quake-source/REPOS.md` | 186-line hand-written rationale per fork | 20 KB | **Move** → `game-dissection/engines/REPOS.md` (already redirects there; cut out the middleman) |
| `tools/quake-source/darkplaces/` | Q1 engine w/ modern renderer | 11 MB / 303 files | **Delete** after confirming variant dir `game-dissection/engines/darkplaces/` carries near-dup diffs |
...
| `game-dissection/REPOS.md` | Index (duplicates tools/quake-source/REPOS.md scope?) | 20 KB | Review vs `engines/SYNTHESIS.md`, merge or delete |
| `game-dissection/engines/_canonical/` | SHA-256-deduped merged tree (authority order wolfcamql-src > quake3e > q3mme > ioquake3 > quake3-source > ...) | 583 MB / 9,895 src files | **KEEP** — this is the target canonical tree |
| `game-dissection/engines/_diffs/` | 513 per-file `.diff.md` for near-dup files across variants | 5 MB | **KEEP** — patch-series source of truth for proto-73 port |
| `game-dissection/engines/_docs/` | Per-engine architecture notes (6 docs for porting targets, 3 docs for ref-only) | 199 KB | **KEEP**, rename → `engines/dissection/` (clearer intent) |
| `game-dissection/engines/_manifest/` | inventory.json, canonical_map.json, build_*.py, DELETE_READY.md, summary.md | 7.6 MB | **KEEP**, rewrite `ROOT` constants to new layout |
| `game-dissection/engines/{darkplaces,demodumper,…yamagi-quake2}/` | 18 per-engine variant dirs holding only the files that DIFFER from `_canonical/` | 24 MB total | **KEEP** — cheap cross-reference |
| `game-dissection/engines/SYNTHESIS.md` | Master synthesis (family tree, authority order, migration paths) | 11 KB | **KEEP**, becomes the unified folder's README |
| `game-dissection/ghidra/` | FT-4 Ghidra RE outputs (binaries/, reports/, symbols/, ipc-maps/, scripts/, projects/) | ~350 MB (binaries staged) | **Move** → `engines/ghidra/` (already under game-dissection, but unify under engines) |
| `game-dissection/assets/` | asset dissection (not engine-specific) | — | **Leave in place** — out of engine scope |
| `game-dissection/graphify-out/` | Knowledge graphs for engine trees (wolfcam/cgame, q3mme/cgame, canonical, …) | — | **Leave in place** — graphify is a tool output, not engine source |
| `game-dissection/.graphify_*.json` | Graphify working files (analysis, detect, extract, labels, semantic, chunk3/5/6) | 3 MB | **Leave in place** — already under game-dissection root |
...
| `.claude/worktrees/hopeful-shtern-7fe850/game-dissection/wolfcam-knowledge-ingest/` | 7 canonical knowledge docs (00-overview..06-proto-73-port-plan) | — | **Cherry-pick into main** at unified folder `engines/wolfcam-knowledge/` |
...
Chosen root name: **`game-dissection/engines/`** (existing — no rename). Rationale:
- It's already the consolidation target. Everything merged into it over 2026-04-17.
- `game-dissection/` is the right semantic home for "we took binaries/source apart to understand them".
- Renaming it now (e.g. to `engine/`) would churn hundreds of cross-refs with no functional gain.

```
G:/QUAKE_LEGACY/game-dissection/
...
│   │   ├── build_inventory.py     # ROOT = game-dissection/engines/variants/
│   │   ├── build_canonical.py     # ROOT = game-dissection/engines/variants/
...
│   ├── ghidra/                    # MOVED from game-dissection/ghidra/
...
- `git mv game-dissection/engines/SYNTHESIS.md game-dissection/engines/README.md`
- **New file** `game-dissection/README.md` linking to `engines/README.md`, `ghidra/README.md` (forward-ref), `assets/`, `graphify-out/`.
...
- Source: `.claude/worktrees/hopeful-shtern-7fe850/game-dissection/wolfcam-knowledge-ingest/` (7 docs + `_scripts/gen.py`).
- Target: `game-dissection/engines/wolfcam-knowledge/`.
- `git checkout hopeful-shtern-branch -- game-dissection/wolfcam-knowledge-ingest/` then `git mv` into final path.
...
- `git mv tools/quake-source/REPOS.md game-dissection/engines/REPOS.md`
...
- `git mv game-dissection/engines/_docs game-dissection/engines/dissection`
- Update `engines/README.md` (was SYNTHESIS) references from `_docs/` to `dissection/`.
- Verification: `grep -r "_docs/" game-dissection/engines/` → no hits.

### Step 5 — Move per-engine variant dirs under `engines/variants/`
- For each of {darkplaces, demodumper, gtkradiant, ioquake3, openarena-engine, openarena-gamecode, q3vm, qldemo-python, quake1-source, quake2-source, quake3-source, quake3e, uberdemotools, wolfet-source, yamagi-quake2, wolfcamql-src, wolfcamql-local-src}: `git mv game-dissection/engines/<name> game-dissection/engines/variants/<name>`.
...
- Verification: `ls game-dissection/engines/variants/ | wc -l` = 17 (16 pure variants + the two wolfcam trees transiting).
...
- `git mv tools/quake-source/q3mme game-dissection/engines/_forks/q3mme`
- **New file** `game-dissection/engines/_forks/README.md` declaring `_forks/` contains writable source trees, not reference clones, and listing q3mme as the proto-73 port target per `docs/research/gate-eng-1-decision-2026-04-19.md`.
...
- Verification: `ls game-dissection/engines/_forks/q3mme/trunk/code/cgame/demos_*.c` → 5 files (demos_camera, demos_capture, demos_cut, demos_effects, demos_fov).

### Step 8 — Retire wolfcam source trees
- Prerequisite: step 6 complete, patches verified, wolfcam-knowledge/ cherry-picked.
- `git rm -r game-dissection/engines/variants/wolfcamql-src`
- `git rm -r game-dissection/engines/variants/wolfcamql-local-src`
...
- Verification: `grep -r "wolfcamql-src\|wolfcamql-local-src" game-dissection/engines/` → hits only in `_canonical/` (expected — the canonical files originated there) + `_diffs/` (patch docs reference the original tree), zero hits in `variants/`.
...
- Create `tools/uberdemotools/`:
  ...
  - Add `tools/uberdemotools/README.md` pointing at `game-dissection/engines/variants/uberdemotools/` for source and at upstream mightycow/uberdemotools GitHub.
- **Bulk deletion** — execute `game-dissection/engines/_manifest/DELETE_READY.md` rm-rf list:
...
- `game-dissection/engines/_manifest/build_inventory.py:17`: `ROOT = Path(r"G:/QUAKE_LEGACY/tools/quake-source")` → `ROOT = Path(r"G:/QUAKE_LEGACY/game-dissection/engines/variants")`
- `game-dissection/engines/_manifest/build_canonical.py:19`: same change
...
- Verification: dry-run `python game-dissection/engines/_manifest/build_inventory.py` — should emit inventory against variants/ and not crash (it scans dirs that still exist).

### Step 12 — Move ghidra/ under engines/
- `git mv game-dissection/ghidra game-dissection/engines/ghidra`
- **New** `game-dissection/engines/ghidra/README.md` with binary-to-source mapping:
  ...
  - etc.
- Update `game-dissection/ghidra/reports/qagamex86_preliminary.md:90` and `_analysis-roadmap.md:13`:
  - `tools/quake-source/wolfcamql-src/code/game/` → `game-dissection/engines/_canonical/code/game/` + cross-ref to `engines/wolfcam-knowledge/patches/`
- Verification: `pytest` + `pyright` still baseline.

### Step 13 — Update `docs/INDEX.md`
- Line 152: `tools/quake-source/ — see REPOS.md` → `game-dissection/engines/ — see README.md`
...
- `game-dissection/engines/variants/wolfcamql-src/` (the per-tree variant dir holding near-dup files — already thin because `_canonical/` got the authoritative copy)
- `game-dissection/engines/variants/wolfcamql-local-src/` (same)
...
|---|---:|---|---|
| `CLAUDE.md` | 507 | `tools/quake-source/wolfcamql-local-src/code/cgame/wolfcam_consolecmds.c` | `game-dissection/engines/wolfcam-knowledge/patches/0007-cgame-wolfcam_*.c-scoreboard-and-FX.patch` (+ narrative pointer) |
| `CLAUDE.md` | 508 | `tools/quake-source/wolfcamql-src/code/cgame/wolfcam_consolecmds.c` | same |
| `CLAUDE.md` | 545 | `tools/quake-source/quake3-source/` | `game-dissection/engines/variants/quake3-source/` |
| `CLAUDE.md` | 546 | `tools/quake-source/q3mme/` | `game-dissection/engines/_forks/q3mme/` |
| `CLAUDE.md` | 547 | `tools/quake-source/quake3e/` | `game-dissection/engines/variants/quake3e/` |
| `CLAUDE.md` | 548 | `tools/quake-source/uberdemotools/` | `game-dissection/engines/variants/uberdemotools/` + `tools/uberdemotools/` (binaries) |
| `docs/INDEX.md` | 152 | `tools/quake-source/` — see REPOS.md | `game-dissection/engines/` — see README.md |
...
| `HUMAN-QUESTIONS.md` | 102 | `tools/quake-source/uberdemotools/UDT_DLL/premake/` | `game-dissection/engines/variants/uberdemotools/UDT_DLL/premake/` |
| `HUMAN-QUESTIONS.md` | 127 | `tools/quake-source/wolfcamql-src/code/` | `game-dissection/engines/_canonical/code/` (authoritative merged copy; + note wolfcam-knowledge/patches/ for the deltas) |
| `docs/reference/dm73-format-deep-dive.md` | 1180 | `G:/QUAKE_LEGACY/tools/quake-source/` | `G:/QUAKE_LEGACY/game-dissection/engines/` |
| `docs/research/proto73-port-review-2026-04-17.md` | 323-325 | three `tools/quake-source/wolfcamql-src/code/...` refs | `game-dissection/engines/_canonical/code/...` (or `engines/variants/wolfcamql-src/code/...` if reader needs wolfcam-specific view) |
...
| `docs/sessions/2026-04-17-wrapup-TOMORROW.md` | 38, 146, 152-154, 175-176 | historical context, keep as-is? | Add a footnote: "As of 2026-04-19 these paths moved to `game-dissection/engines/` — see engine-folder-unification-plan-2026-04-19.md" |
| `docs/specs/highlight-criteria-v1.md` | 197 | `tools/quake-source/uberdemotools/UDT_DLL/` | `game-dissection/engines/variants/uberdemotools/UDT_DLL/` |
| `docs/visual-record/2026-04-17/steam-paks-proof/README.md` | 48 | `tools/quake-source/quake3-source/` | `game-dissection/engines/variants/quake3-source/` |
| `game-dissection/ghidra/reports/qagamex86_preliminary.md` | 90 | `tools/quake-source/wolfcamql-src/code/game/` | `game-dissection/engines/_canonical/code/game/` |
| `game-dissection/ghidra/reports/_analysis-roadmap.md` | 13 | `tools/quake-source/wolfcamql-src/` | `game-dissection/engines/wolfcam-knowledge/patches/` (+ note that source tree is retired, patches are the oracle now) |
| `game-dissection/ghidra/reports/_binary-inventory.md` | 28, 30, 42, 43 | `tools/quake-source/...` | path rewrites per above table |
| `game-dissection/engines/_manifest/build_inventory.py` | 17 | `ROOT = Path(r"G:/QUAKE_LEGACY/tools/quake-source")` | `ROOT = Path(r"G:/QUAKE_LEGACY/game-dissection/engines/variants")` |
| `game-dissection/engines/_manifest/build_canonical.py` | 19, 200, 223 | same + rm-rf template | `ROOT = variants/`; rm-rf template writes to variants/ |
| `game-dissection/engines/_manifest/DELETE_READY.md` | 20-37 | rm-rf list | Keep as archival section; add "DONE 2026-04-19" header above; add new section for the post-migration state |
...
FT-4 Ghidra work is the live RE track. Per `game-dissection/ghidra/reports/_binary-inventory.md`, 9 binaries are staged under `binaries/` — wolfcamql-11.1.exe + 11.3.exe + 5 DLLs + StartupWizard.exe + WolfWhisperer.exe.

**Where FT-4 outputs land in the unified folder:**

```
game-dissection/engines/ghidra/
...
- Every reference in CLAUDE.md, HUMAN-QUESTIONS.md, docs/INDEX.md, reference docs, research docs, session docs, and the two `_manifest/*.py` scripts points at `game-dissection/engines/...`.
...
ls game-dissection/engines/_forks/q3mme/trunk/code/cgame/        # 6. q3mme live fork present
ls game-dissection/engines/wolfcam-knowledge/patches/*.patch | wc -l  # 7. ≥7 patches
ls game-dissection/engines/ghidra/                                # 8. ghidra moved under engines
```
...
1. **One folder:** `game-dissection/engines/` becomes the single unified home for all engine source, dissection docs, binary RE, patches, and knowledge. `tools/quake-source/` disappears entirely.
...
4. **Ghidra moves under engines/:** `game-dissection/ghidra/` → `game-dissection/engines/ghidra/` with a binary-to-source map linking decompilation to source+patch truth.
