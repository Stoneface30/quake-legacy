...
- [ ] **Step 1: Upstream — already established (2026-09-12).** `brugal/wolfcamql` publishes releases only from v11.7 onward, so no upstream asset carries these bytes. But the tag **`v11.3` → commit `73e2d707e5dd1fb0fc50d4ad9f00940909c4b3ec`** contains every one of the archive's 929 files **byte-identical**. The tag's only extras are packaging: `.gitignore`, `code/botlib/lcc.mak`, `code/botlib/linux-i386.mak`, `code/qcommon/version.h`, `macwolfcambuild/`, `package-files/`, `track-svn.txt`, `update-mac-binaries.sh`.

  So the bootstrap's **primary source is upstream by commit**, verified against a **per-file manifest** rather than an archive hash. GitHub's generated tarballs are not byte-stable, but file contents at a commit are immutable. No project Release is needed unless upstream becomes unreachable (then Step 6's ask applies). The pinned local archive (`WOLFCAMQL_ARCHIVE`, SHA-256 above) remains the offline fallback.

  - Generate `third_party/wolfcamql/TREE.sha256` from the pinned archive: one line per regular file, `<sha256>  <path relative to wolfcamql-src/>`, sorted. Commit it.
  - `SOURCE.json` gains `"upstream_commit": "73e2d707e5dd1fb0fc50d4ad9f00940909c4b3ec"`, `"upstream_tarball": "https://codeload.github.com/brugal/wolfcamql/tar.gz/73e2d707e5dd1fb0fc50d4ad9f00940909c4b3ec"`, `"tree_manifest": "third_party/wolfcamql/TREE.sha256"`, and `"upstream_extras_ignored"` listing the 8 entries above.
  - The bootstrap's `_obtain` order becomes: local archive (archive SHA-256) → cache → **upstream tarball**: download to a temp file, extract into a staging dir with the same safe-member rules, then verify *every* manifest path exists with its SHA-256 and that nothing exists beyond the manifest except `upstream_extras_ignored`, then rename the single top directory to `wolfcamql-src`. Any mismatch raises `ChecksumMismatch` before the staging dir moves into place. Add a unit test for the manifest path (a two-file tree, one byte altered → refused).
  - **Local edit to undo first.** Today's extracted `renderer/tr_init.c` differs from both the archive and the tag by **one added blank line at line 2234** (file dated 2026-09-07 18:46). It was edited in place and git cannot see it. Before `--adopt`, restore that file from the archive: `tar -xzOf "$WOLFCAMQL_ARCHIVE" wolfcamql-src/code/renderer/tr_init.c > <tree>/code/renderer/tr_init.c`. Then adopt; every file must match. Record the finding in `provenance_notes`.
