...
def derive(wins: list[dict], name_to_hash: dict[str, str]) -> tuple[list[dict], list[dict]]:
    """(all_players, recorder) from clutch round wins. Pure; unit-tested."""
    ...
    recorder = [r for r in all_players if ce.is_alias(r.get("player_name"))]
    return all_players, recorder
...
def main() -> int:
    ...
    demos = con.execute("select name, coalesce(duplicate_of, content_hash), path "
                        "from demos where parse_error is null").fetchall()
    con.close()
    name_to_hash = {n: h for n, h, _p in demos}
    paths = [DATA / "demos" / n for n, _h, _p in demos]
    ...
    all_players, recorder = derive(wins, name_to_hash)
    ...
    _write(OUT_DIR / "clutch_all_players.csv", all_players, JOINED_COLS)
    _write(OUT_DIR / "clutch_recorder.csv", recorder, JOINED_COLS)
    print(f"[clutch] wins {len(wins)}  all_players {len(all_players)}  "
          f"recorder {len(recorder)}  failed {failed}", flush=True)
