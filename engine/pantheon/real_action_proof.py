...
def main() -> int:
    ...
    report = {
        ...
        "context_actors": getattr(scn, "context_actors", {}),
    ...
    ev = diff["events"]
    print(f"  events       {ev['verdict']}: real={len(ev['real'])} "
          f"missing={ev['missing_from_synthetic']}")
    print(f"  context      {report['context_actors']}")
    return 0
