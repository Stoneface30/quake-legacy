...
### Rule HL-8: Rendering requires explicit permission (user requirement, 2026-09-05)
- **WHAT** Every game-process launch (capture_demo, director_preview, director_session, _preview_job, engine supervisor, shot.render, cvar_probe, playback_probe) calls `creative_suite/engine/render_permit.require(purpose)` immediately before spawning. Default: **DENY**. The one operator setting is `PANTHEON_RENDER_ALLOWED=1`; even then a running protected game (`quakelive*.exe`, `PANTHEON_PROTECTED_GAMES`) DEFERS the launch; `PANTHEON_RENDER_FORCE=1` is the separate explicit override. A denied queue job stays `QUEUED` with `RENDER DEFERRED: <reason>`, never FAILED; a READY proxy keeps playing. Do not add a second permission setting.
- **WHERE** `creative_suite/engine/render_permit.py` · `creative_suite/tests/test_render_permit.py` (every launch site must call it; a new `Popen` of wolfcamql elsewhere fails the suite) · `creative_suite/tests/conftest.py` (no test can create a game process) · `docs/reference/pantheon_ownership.md`.
- **WHY** A review origin coming up reclaimed queued proxy jobs and launched Wolfcam over the game the user was playing. `SW_SHOWMINNOACTIVE` is defence in depth; a process that must not run is not launched.
