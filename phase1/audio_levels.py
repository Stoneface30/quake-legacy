...
def extract_music_stem(
    ...
    """Write a WAV stem of the music track used in the render.

    Cheap implementation: copy-converts `music_src` (the exact file fed to the
    render) to PCM WAV. Downstream loudness measurement is identical whether
    we re-extract from the mp4 or copy the source — this avoids a lossy
    decode-then-source-separate step.
    """
    out = Path(out_wav)
